...
class Prefs(ctx: Context) {
    ...
    fun setLastStudentPull(ts: Instant) {
        p.edit().putString("lastStudentPull", ts.toString()).apply()
    }

    fun setLastScorePull(ts: Instant) {
        p.edit().putString("lastScorePull", ts.toString()).apply()
    }
}
